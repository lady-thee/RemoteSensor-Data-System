...
class SensorFactory(factory.django.DjangoModelFactory):
    ...
    name = factory.Faker('random_element', elements=['WeatherTracker', 'MeteorSense', 
                                                     'ClimateProbe', 'AtmosData', 
                                                     'SkyWatch', 'AeroMetrix', 
                                                     'HumidiProbe', 'BaroGuard', 
                                                     'PrecisioWeather', 'CloudSense', 
                                                     'StormSense', 'AeroPulse', 'SolarClima'
                                                     ])
    description = factory.LazyAttribute(lambda x: f"A weather sensor for {x.operator.company_name} that captures and reports atmospheric conditions.")
    data_format = factory.Faker('random_element', elements=['json, floats', 'json,floats,text'])
    communication_mode = factory.Faker('random_element', elements=['wireless', 'satellite'])
...
class SensorDataFactory(factory.django.DjangoModelFactory):
    ...
    wind_direction = factory.Faker('random_element', elements=['EWS', 'NWS', 'NSE', 'SWS', 'SWE', 'ENS', 'SEN', 'WSW'])
    rainfall = factory.Faker('pyfloat', left_digits=2, right_digits=2)
    weather_id = factory.Faker('pyint')
    main_weather = factory.Faker('random_element', elements=['sunny', 'cloudy', 'rainy', 'stormy', 'foggy', 'snowy'])
    weather_description = factory.Faker('random_element', elements=['Today is sunny', 'Today is cloudy', 'Today is rainy',
                                                                    'Today is stormy', 'Today is foggy', 'Today is snowy'])
    ...
    city_name = factory.Faker('random_element', elements=['New York City, USA', 'Tokyo, Japan', 'London, United Kingdom',
                                                          'Paris, France', 'Sydney, Australia', 'Cape Town, South Africa', 
                                                          'Lagos, Nigeria', 'Cairo, Egypt', 'Rio de Janeiro, Brazil', 
                                                          'Mumbai, India', 'Beijing, China', 'Moscow, Russia', 'Istanbul, Turkey', 
                                                          'Mexico City, Mexico', 'Nairobi, Kenya', 'Casablanca, Morocco', 'Accra, Ghana'])
