...
class UserLoginAPIView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            return Response(
                {"message": "Logged In Successfully!", "token": str(token.key)},
                status.HTTP_200_OK,
            )
        else:
            return Response(
...
class RetrieveUpdateDeleteUserAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        if not self.request.user.is_authenticated:
            return Response(
                {"detail": "You have to log in to view this page!"},
                status.HTTP_401_UNAUTHORIZED,
            )
        user_email = self.request.user.email
        user_id = (
            User.objects.filter(email=user_email).values_list("id", flat=True).get()
        )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Account Deleted Successfully!"}, status.HTTP_204_NO_CONTENT
        )
...
class RetrieveUpdateDeleteSensorAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Sensor Deleted Successfully!"}, status.HTTP_204_NO_CONTENT
        )
